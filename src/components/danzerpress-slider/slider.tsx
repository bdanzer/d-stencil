...
export class Slider {
  ...
  @Prop() childsClassName: string = '.dp-slide'; // String : slider child slides elements
  ...
  @Method()
  async goToNext(e?: any) {
    e && e.preventDefault();
    this.slideTo(this.current - this.operator );
  }

  @Method()
  async goToPrevious(e?: any) {
    e && e.preventDefault();
    this.slideTo(this.current + this.operator );
  }
  ...
  async slideTo(n: number) {
    ...
    console.log(this.operator, this.current, this.slideWidth);
  ...
  render() {
    let html = [
      ...
      this.showButtons && <div class="carousel-buttons">
        <a href="" class="next carousel-arrow" onClick={this.goToNext.bind(this)}></a>
        <a href="" class="prev carousel-arrow" onClick={this.goToPrevious.bind(this)}></a>
      </div>
    ];
    return html;
  }
